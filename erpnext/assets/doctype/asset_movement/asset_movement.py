...
class AssetMovement(Document):
	...
	def validate(self):
		if self.purpose == 'Receipt':
			return
		self.old_state = frappe.get_value("Asset Movement", self.name, "workflow_state")
		self.validate_asset()
	...
	def manual_workflow_states(self):
		if self.workflow_state == "Draft":
			return
		if self.old_state.lower() == "Waiting For Verification".lower():
			for d in self.assets:
				#if single asset is not verified. keep under verification state
				if d.is_verified == 0:
					self.workflow_state = self.old_state
					# frappe.msgprint("")
		elif self.workflow_state.lower() == "Waiting Approval".lower():
			if self.email != frappe.session.user:
				frappe.throw(f"Only receiving person {self.to_employee_name} can verify it.")
		
		elif self.workflow_state.lower() in ("Approved".lower()):
			if not "Asset Manager" in frappe.get_roles(frappe.session.user) or "Administrator" == frappe.session.user:
				frappe.throw("Only users with role Asset Manager can Approve this document")

	def manual_notify_workflow_states(self):
		employee = frappe.get_doc("Employee", frappe.db.get_value("Employee",{"user_id":self.owner},"name"))
		if not employee:
			frappe.throw(f"A valide employee can only make the Transaction")
		template = frappe.db.get_single_value('HR Settings', 'asset_movement_notification_template')
		if not template:
			frappe.msgprint(_("Please set default template for Asset Movement Status Notification in Asset Settings."))
			return
		receipients = employee.user_id
		if self.workflow_state=='Waiting For Verification':
			receipients = [frappe.get_value("Employee", x.to_employee, "user_id") for x in self.assets]
			receipients = list(dict.fromkeys(receipients))
		elif self.workflow_state=='Waiting Approval':
			""" notify all Asset Manager roles """
			users_with_asset_role = frappe.get_all("Has Role", filters={"role": "Asset Manager", "parenttype": "User", "parent": ("not in", ['Administrator','auditor@cdcl.bt', 'sangay.choden@thimphutechpark.bt', 'yeshi.dorji@cdcl.bt', 'pralad.rai@thimphutechpark.bt', \
				'sonam.zangmo@thimphutechpark.bt', 'sangayjams72@gmail.com', 'draugyon18@gmail.com', 'chenchodorji012@gmail.com', 'sonampelzang.cdcl@gmail.com', 'ugyen.phuntsho@cdcl.bt', 'kenchorabten@erp.bt', 'sangaytempa1999@gmail.com', 'chophel.tracp@gmail.com', \
				'demakarma856@gmail.com', 'gyenzin@cdcl.bt', 'kalden.dorji@thimphutechpark.bt', 'phuntsho.sonam@cdcl.bt', 'jharna.pradhan@cdcl.bt', 'jangchub.gyeltshen@cdcl.bt', 'deki.wangmo@thimphutechpark.bt', 'jigme.wangdi@cdcl.bt', 'lhendrup.dorji@thimphutechpark.bt', \
				'tshering.lham@thimphutechpark.bt', 'sonam.bomjan@thimphutechpark.bt', 'jairaj.rai@thimphutechpark.bt', 'sonam.choki@thimphutechpark.bt'])}, fields=["parent"])
			if users_with_asset_role:
				receipients = [a['parent'] for a in users_with_asset_role if frappe.db.get_value("User", a['parent'], "enabled") == 1]
