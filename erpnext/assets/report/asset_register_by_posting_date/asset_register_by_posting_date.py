...
def get_data(filters):
    ...
    if filters.get("posting_date"):
        posting_date_condition = "AND a.posting_date = '{posting_date}'".format(
            posting_date=filters.posting_date
        )
    else:
        posting_date_condition = "AND a.posting_date BETWEEN '{from_date}' AND '{to_date}'".format(
    ...
    if filters.cost_center:
        query += " AND a.cost_center = '{0}'".format(filters.cost_center)

    if filters.asset_category:
        query += " AND a.asset_category = '{0}'".format(filters.asset_category)
