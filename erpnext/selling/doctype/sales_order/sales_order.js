...
frappe.ui.form.on("Sales Order", {
	...
	refresh: function (frm) {
		...
		// Auto fetch uom when mapping from other doctype
		frm.doc.items.forEach(row => {
            if (row.item_code && !row.uom) {
                frappe.db.get_value('Item', row.item_code, 'stock_uom')
                    .then(r => {
                        if (r.message && r.message.stock_uom) {
                            frappe.model.set_value(row.doctype, row.name, 'uom', r.message.stock_uom);
                        }
                    });
            }
        });
		...
		if (doc.docstatus == 1) {
			...
			if (doc.status !== 'Closed') {
				...
				// if (flt(doc.per_billed) < 100) {
					...
					this.frm.add_custom_button(__('Advance Payment'), () => this.make_payment_entry(), __('Create'));
				// }
