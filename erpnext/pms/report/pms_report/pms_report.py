...
def get_data(filters):
    ...
    if filters.get("type") == "Target Setup Report":
        query = """
            SELECT
                ...
                branch,
                date
            FROM `tabTarget Set Up`
            {}
        """.format(cond)

        #Revew report
    elif filters.get("type") == "Review Report":
    ...
    elif filters.get("type") == "Performance Evaluation Report":
...
def get_column(filters):
    ...
    columns = [
        ...
        _("Employee") + ":Data:120",
        _("EAS Group") + ":Data:120",
        _("Designation") + ":Data:120",
        _("EAS Calendar") + ":Data:120",
        _("Approver Name") + ":Data:120",
        _("Branch") + ":Data:120",
    ...
    if filters.get("type") == "Target Setup Report":
        ...
        columns.append(_("Posting Date") + ":Date:120")

    elif filters.get("type") == "Review Report":

        columns.append(_("Review Date") + ":Date:120")

    elif filters.get("type") == "Performance Evaluation Report":

        columns.extend([
            _("Reviewer Name") + ":Data:120",
            _("Review") + ":Data:120",
            _("Evaluation Date") + ":Date:120",
           
            _("Final Score Percent") + ":Float:120",
            # _("Final Score (%)") + ":Float:120",
            _("Overall Rating") + ":Data:120"
        ])

    # elif filters.get("type") == "PMS Summary":

    #     columns.extend([
    #         _("Date Of Joining") + ":Date:120",
    #         _("Posting Date") + ":Date:120",
    #         _("Form I Total Rating") + ":Float:120",
    #         _("Form II Total Rating") + ":Float:120",
    #         _("Form I Score") + ":Float:120",
    #         _("Form II Score") + ":Float:120",
    #         _("Final Score") + ":Float:120",
    #         _("Final Score (%)") + ":Float:120",
    #         _("Overall Rating") + ":Data:120"
    #     ])
...
def get_conditions(filters):
    ...
    if filters.get("eas_calendar"):
        conditions.append(
            "eas_calendar = '{}'".format(filters.get("eas_calendar"))
        )
    ...
    # TARGET SETUP REPORT
    # -------------------------
    if report_type == "Target Setup Report":

        if filters.get("workflow_state"):
            conditions.append(
                "workflow_state = '{}'".format(filters.get("workflow_state"))
            )

    # -------------------------
    # REVIEW REPORT
    # -------------------------
    elif report_type == "Review Report":
    ...
    # -------------------------
    # PERFORMANCE EVALUATION (FIXED)
    # -------------------------
    elif report_type == "Performance Evaluation Report":
        ws = filters.get("workflow_state")
        
        if ws:
            conditions.append("""
            eval_workflow_state = '{0}'
        """.format(ws))
    ...
    if filters.get("branch"):
        conditions.append(
            "branch = '{}'".format(filters.get("branch"))
        )
