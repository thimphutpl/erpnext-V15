...
class EquipmentHiringExtension(Document):
    ...
    def update_date(self, submit=None):
        name = frappe.db.sql(
            "select ha.name from `tabHiring Approval Details` ha, `tabEquipment Hiring Form` h "
            "where ha.parent = h.name and h.docstatus = 1 and ha.equipment = %s and h.name = %s",
            (str(self.equipment), str(self.ehf_name)), as_dict=True)
        if name:
                ...
                if str(doc.to_date) >= self.extension_date:
                    ...
                    if submit:
                        res = frappe.db.get_value(
                            ...
                            {"equipment": doc.equipment, "ehf_name": doc.parent, "from_date": doc.from_date,
                             "to_date": doc.to_date, "docstatus": 1},
                            "name"
                        )
                        ...
                        if ehf.private == 'Private':
                            balance_advance = frappe.db.sql(
                                "select sum(credit_in_account_currency) as amount "
                                "from `tabJournal Entry Account` "
                                "where reference_type = 'Equipment Hiring Form' and reference_name = %s "
                                "and docstatus = 1 and is_advance = 'Yes'",
                                self.ehf_name, as_dict=True
                            )
                            if balance_advance:
    ...
    def check_hire_rate(self, doc):
        ...
        db_query = (
            "select {0} as rate_fuel, {1} as rate_wofuel, {2} as idle_rate, "
            "a.yard_hours, a.yard_distance "
            "from `tabHire Charge Item` a, `tabHire Charge Parameter` b "
            "where a.parent = b.name and b.equipment_type = '{3}' and b.equipment_model = '{4}' "
            "and '{5}' between a.from_date and ifnull(a.to_date, now()) "
            "and '{6}' between a.from_date and ifnull(a.to_date, now()) LIMIT 1"
        )
    ...
    def post_journal_entry(self):
        ...
        if revenue_bank and advance_account:
            ...
            je.title = f"Advance for Equipment Extension Hire ({self.ehf_name})"
            je.voucher_type = 'Bank Entry'
            je.naming_series = 'Bank Receipt Voucher'
            je.remark = f'Advance payment against: {self.name}'
            ...
            frappe.msgprint(f"Posted an advance amount of {self.total_amount} for {self.hours} hours")
