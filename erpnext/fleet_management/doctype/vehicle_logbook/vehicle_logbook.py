...
class VehicleLogbook(AccountsController):
	...
	if TYPE_CHECKING:
		from erpnext.fleet_management.doctype.vehicle_log.vehicle_log import VehicleLog
		...
		vehicle_logbook: DF.Literal["", "Pool Vehicle", "Tractor and Truck", "Revenue Earning Equipment", "Equipment Hiring Form"]
...
def get_equipment_data(equipment_name, all_equipment=0, branch=None):
	...
	query = """
		SELECT e.name, e.branch, e.equipment_name, e.hsd_type, e.equipment_type, e.equipment_model
		FROM `tabEquipment` e
		JOIN `tabEquipment Type` et ON e.equipment_type = et.name AND e.branch = %(branch)s
	"""
