...
frappe.ui.form.on('Vehicle Logbook', {
    ...
    ehf_name: function (frm) {
		...
		if (frm.doc.ehf_name) {
			frappe.call({
				...
				args: { form: frm.doc.ehf_name },
				callback: function (r) {
					if (r.message) {
						frm.set_value("equipment", r.message[0].equipment);
	...
	refresh: function(frm) {
        ...
        if (frm.doc.branch && frm.doc.vehicle_logbook) {
            frm.set_df_property('equipment', 'hidden', false);
			frm.set_df_property('equipment', 'read_only', true);
        }
        else if (frm.doc.vehicle_logbook === 'Pool Vehicle') {
            frm.set_df_property('equipment', 'hidden', false);
			frm.set_df_property('equipment', 'read_only', true);
        }
		else if (frm.doc.vehicle_logbook === 'Tractor and Truck') {
            frm.set_df_property('equipment', 'hidden', false);
			frm.set_df_property('equipment', 'read_only', true);
        }
		else if (frm.doc.vehicle_logbook === 'Revenue Earning Equipment') {
            frm.set_df_property('equipment', 'hidden', false);
			frm.set_df_property('equipment', 'read_only', true);
		}
		else if (frm.doc.vehicle_logbook == 'Equipment Hiring Form') {
            frm.set_df_property('equipment', 'hidden', false);
            frm.set_df_property('equipment', 'read_only', true);
        } else {
            frm.set_df_property('equipment', 'hidden', true);
        }
	},
    ...
    vehicle_logbook: function (frm) {
		if (["Pool Vehicle"].includes(frm.doc.vehicle_logbook)) {
		...
		if (frm.doc.vehicle_logbook === 'Equipment Hiring Form') {
			...
			frm.set_df_property('equipment', 'hidden', false);
			frm.set_df_property('equipment', 'read_only', false);
			frm.set_query('equipment', function () {
				return {
					filters: {
			...
			frm.set_query('equipment', function () {
				return {
					filters: {
						equipment_category: 'Tractor and Truck'
					}
				};
			});
			frm.trigger('equipment'); // Trigger equipment logic
			...
			frm.set_query('equipment', function () {
				return {
					filters: {
						equipment_category: 'Revenue Earning Equipment'
					}
				};
			});
			frm.trigger('equipment'); // Trigger equipment logic
...
cur_frm.add_fetch("equipment", "equipment_name", "equipment_name")
