...
class POLReceive(StockController):
	...
	def before_save(self):
		...
		if self.book_type == "Own" and flt(self.tank_capacity) < flt(self.tank_balance + self.qty) and self.equipment_branch !="GI - Head Office":
			frappe.throw(
	...
	def update_stock_ledger(self):
		...
		sl_entries.append(self.get_sl_entries(self, {
					...
					"warehouse": wh, 
					"incoming_rate": round(flt(self.total_fuel_amount) / flt(self.qty) , 2)
				}))
	...
	def get_gl_entries(self, warehouse_account):
		...
		gl_entries.append(
			self.get_gl_dict({"account": expense_account,
					 "debit": flt(self.total_fuel_amount),
					 "debit_in_account_currency": flt(self.total_fuel_amount),
