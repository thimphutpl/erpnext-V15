...
class FabricationAndBaileyBridge(AccountsController):
	...
	def check_items(self):
		if not self.items:
			frappe.throw("Cannot submit fabrication and bailey bridge with empty job details")
		else:
			for a in self.items:
				if flt(a.amount) == 0: 
					frappe.throw("Cannot submit fabrication and bailey bridge without cost details")
	...
	def update_reservation(self):
		frappe.db.sql("update `tabEquipment Reservation Entry` set to_date = %s, to_time = %s where docstatus = 1", (self.finish_date, self.job_out_time))
		frappe.db.sql("update `tabEquipment Status Entry` set to_date = %s, to_time = %s where docstatus = 1 ", (self.finish_date, self.job_out_time))							

	@frappe.whitelist()
	def get_job_items(self):
		items = frappe.db.sql("select se.name as stock_entry, sed.item_code as job, sed.item_name as job_name, sed.qty as quantity, sed.amount from `tabStock Entry Detail` sed, `tabStock Entry` se where se.docstatus = 1 and sed.parent = se.name and se.purpose = \'Material Issue\' and se.fabrication_bailey_bridge = \'"+ str(self.name) +"\'", as_dict=True)
		...
		if items:
			...
			frappe.msgprint("No stock entries related to the fabrication and bailey bridge found. Entries might not have been submitted?")

@frappe.whitelist()
def make_bank_entry(frm=None):
	if frm:
		job = frappe.get_doc("Fabrication And Bailey Bridge", frm)
...
def make_payment_entry(source_name, target_doc=None): 
	def update_docs(obj, target, source_parent):
		...
		target.payment_for = "Fabrication And Bailey Bridge"
	...
	doc = get_mapped_doc("Fabrication And Bailey Bridge", source_name, {
			"Fabrication And Bailey Bridge": {
				...
				"validation": {"docstatus": ["=", 1], "fabrication_and_bailey_bridge": ["is", None]}
			},
		}, target_doc)
	return doc
