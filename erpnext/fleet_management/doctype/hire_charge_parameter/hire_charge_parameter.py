...
class HireChargeParameter(Document):
	...
	if TYPE_CHECKING:
		...
		benchmark: DF.Float
		broadleaf_external: DF.Currency
		broadleaf_internal: DF.Currency
		conifer_external: DF.Currency
		conifer_internal: DF.Currency
		equipment: DF.Link | None
		equipment_model: DF.Link
		equipment_name: DF.Data | None
		equipment_type: DF.Link
		idle_external: DF.Currency
		idle_internal: DF.Currency
		interval: DF.Float
		items: DF.Table[HireChargeItem]
		kph: DF.Float
		lph: DF.Float
		with_fuel_external: DF.Currency
		with_fuel_internal: DF.Currency
		without_fuel_external: DF.Currency
	...
	def validate(self):
		...
		if existing_parameter:
			frappe.throw(
				f"Hire Charge Parameter already exists for Equipment '{self.equipment_name}'. "
	...
	def set_parameter_values(self):
		# p = frappe.db.sql("select name from `tabHire Charge Parameter` where equipment_name = %s and equipment_type = %s and equipment_model = %s and name != %s", str(self.registeration_number), (str(self.equipment_type), str(self.equipment_model), str(self.name)), as_dict=True)
		p = frappe.db.sql(
			"""
			SELECT name 
			...
			AND name != %s
			""", 
			(self.equipment_name, self.equipment_type, self.equipment_model, self.name),
			as_dict=True
		)
		if p:
			frappe.throw("Hire Charges for the equipment type and model already exists. Update " + str(p[0].name))
		if self.items:
			self.db_set("without_fuel_external", self.items[len(self.items) - 1].rate_wofuel_external)	
			self.db_set("with_fuel_external", self.items[len(self.items) - 1].rate_fuel_external)	
			self.db_set("idle_external", self.items[len(self.items) - 1].idle_rate_external)	
			self.db_set("broadleaf_external", self.items[len(self.items) - 1].rate_broadleaf_external)	
			self.db_set("conifer_external", self.items[len(self.items) - 1].rate_conifer_external)	
			...
			self.db_set("broadleaf_internal", self.items[len(self.items) - 1].rate_broadleaf_internal)
			self.db_set("conifer_internal", self.items[len(self.items) - 1].rate_conifer_internal)
