...
class HSDPayment(Document):
		...
		for a in self.items:
			...
			if doc:
				...
				if cancel:
					...
					max_amount = round(flt(doc.total_gst_amount,2) if doc.get("total_gst_amount") else flt(doc.total_amount,2))
					if flt(paid_amount) > flt(max_amount):
	...
	def get_invoices_without_gst(self):
		if not self.fuelbook:
			frappe.throw("Select a Fuelbook to Proceed")
		query = "select name as pol, pol_type as pol_item_code,included_gst,outstanding_amount as amount_without_gst,total_amount as payable_amount, item_name, memo_number from `tabPOL Receive` where docstatus = 1 and outstanding_amount > 0 and fuelbook = %s order by posting_date, posting_time"
		entries = frappe.db.sql(query, self.fuelbook, as_dict=True)
		self.set('items', [])
