...
#         )
#         d.rate = rate and flt(rate[0][0]) or 0.0
#         d.hsd_amount = round(flt(d.rate) * flt(d.consumed), 2)
...
#             WHERE hcp.name = hci.parent AND hcp.equipment_type = %s AND hcp.equipment_model = %s 
#             AND (%s BETWEEN from_date AND IFNULL(to_date, CURDATE()) 
#             OR %s BETWEEN from_date AND IFNULL(to_date, CURDATE()))""",
#             (d.equipment_type, d.equipment_model, filters.from_date, filters.to_date),
#             as_dict=1,
#         )
#         d.yskm = ys_records and flt(ys_records[0].yard_distance) or 0
#         d.yshour = ys_records and flt(ys_records[0].yard_hours) or 0
...
#             d.place,
#             f"{d.open_km}/{d.open_hr}",
#             f"{d.close_km}/{d.close_hr}",
#             round(d.close_km - d.open_km, 2),
#             round(d.close_hr - d.open_hr, 2),
#             round(flt(d.drawn), 2),
#             round(flt(d.opening), 2),
#             round(flt(d.drawn) + flt(d.opening), 2),
#             d.yskm,
#             d.yshour,
#             round(d.consumed, 2),
