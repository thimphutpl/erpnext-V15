...
from frappe.utils import cstr, flt, getdate, today, nowdate, now_datetime, cint
...
def get_boq_items_as_dict(
	...
	item_dict = {}
	# frappe.throw(str(boq))
	# Did not use qty_consumed_per_unit in the query, as it leads to rounding loss
	query = """select
				boq_item.item_code,
				boq_item.idx,
				item.item_name,
				sum(boq_item.quantity) * %(qty)s as qty,
				item.image,
				boq.project,
				boq_item.rate,
				sum(boq_item.{qty_field}) * boq_item.rate * %(qty)s as amount,
				item.stock_uom,
				item.item_group,
			...
			from
				`tab{table}` boq_item
				JOIN `tabBOQ` boq ON boq_item.parent = boq.name
				JOIN `tabItem` item ON item.name = boq_item.item_code
				LEFT JOIN `tabItem Default` item_default
					ON item_default.parent = item.name and item_default.company = %(company)s
			where
				boq_item.docstatus < 2
				and boq.name = %(boq)s
	...
	# 	items = frappe.db.sql(
	# 		query, {"parent": boq, "qty": qty, "bom": boq, "company": company}, as_dict=True
	# 	)
	...
	# 	items = frappe.db.sql(query, {"qty": qty, "boq": boq, "company": company}, as_dict=True)
	# else:
	query = query.format(
		table="BOQ Item",
		where_conditions="",
