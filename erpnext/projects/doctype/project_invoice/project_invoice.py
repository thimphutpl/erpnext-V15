...
class ProjectInvoice(AccountsController):
	...
	if TYPE_CHECKING:
		from erpnext.projects.doctype.project_invoice_advance.project_invoice_advance import ProjectInvoiceAdvance
		from erpnext.projects.doctype.project_invoice_boq.project_invoice_boq import ProjectInvoiceBOQ
		from erpnext.projects.doctype.project_invoice_deduction.project_invoice_deduction import ProjectInvoiceDeduction
		from erpnext.projects.doctype.project_invoice_mb.project_invoice_mb import ProjectInvoiceMB
		from erpnext.projects.doctype.project_invoice_rebate_item.project_invoice_rebate_item import ProjectInvoiceRebateItem
		from frappe.types import DF

		advance_recovery: DF.Currency
		advances: DF.Table[ProjectInvoiceAdvance]
		...
		debit_credit_account: DF.Link
		deductions: DF.Table[ProjectInvoiceDeduction]
		...
		rebate: DF.Table[ProjectInvoiceRebateItem]
		...
		total_deduction_amount: DF.Currency
		total_paid_amount: DF.Currency
		total_received_amount: DF.Currency
		type: DF.Literal["", "Domestic Vendor", "International Vendor", "Rent and Consultancy", "Dividend"]
