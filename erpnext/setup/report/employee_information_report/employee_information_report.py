...
def get_data(filters):
	...
	data = frappe.db.sql("""
		...
		FROM `tabEmployee` e where 1=1 {0}
		""".format(conditions))
	# frappe.throw(str("""
	# 	SELECT e.name, e.employee_name, e.passport_number, e.company_email, e.date_of_birth, e.cell_number, e.reports_to, 
	# 	(select t.employee_name from `tabEmployee` t where t.name=e.name) as reports_to_name,
...
def get_conditions(filters):
	conditions = ''
	if filters.get("employee"): conditions += " and e.employee = '{0}'".format(filters.employee)
	if filters.get("date_of_joining"):
		from_date = str(filters["date_of_joining"][0])  # From Date
		to_date = str(filters["date_of_joining"][1])
		
		conditions+=" and e.date_of_joining BETWEEN '{0}' AND '{1}'".format(from_date,to_date)
	if filters.get("branch"): conditions += " and e.branch = '{0}'".format(filters.branch)
	if filters.get("status") == "All":
			conditions += " and e.status = e.status"
	elif filters.get("status") == "Active":
			conditions += " and e.status = 'Active'"
	elif filters.get("status") == "In Active":
			conditions += " and e.status = 'In Active'"
