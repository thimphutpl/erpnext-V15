...
def get_holiday_list_for_employee(employee, raise_exception=True):
	if employee:
		branch, holiday_list, company = frappe.db.get_value(
			"Employee", employee, ["branch", "holiday_list", "company"]
		)
	else:
		branch = None
		holiday_list = None
	...
	if branch:
		branch_holiday_list = frappe.db.get_value("Branch", branch, "holiday_list")
		if branch_holiday_list:
			return branch_holiday_list
	...
	if raise_exception:
		frappe.throw(
			_("Please set a Holiday List for Branch, Employee {0}, or Company {1}")
...
def get_permission_query_conditions(user):
	...
	if "HR User" in user_roles or "HR Manager" in user_roles or "Accounts User" in user_roles or "HR Support" in user_roles:
		return
...
def has_record_permission(doc, user):
	...
	if "HR User" in user_roles or "HR Manager" in user_roles or "HR Support" in user_roles or "Accounts User" in user_roles:
		return True
	else:			
