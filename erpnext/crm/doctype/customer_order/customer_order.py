...
class CustomerOrder(Document):
    ...
    def make_sales_order(self):
        ...
        if self.selection_based_on != "Lot" and self.selection_based_on != "Measurement":
            ...
            doc = frappe.get_doc({
                ...
                # "taxes_and_charges": "5% GST Outward",
                "items": [{
                    "item_code": self.item,
    ...
    def update_payable_amount(self):
        ...
        if self.selection_based_on != "Lot" and self.selection_based_on != "Measurement":
            ...
            self.gst_amount = (flt(flt(flt(self.total_item_rate) + flt(self.total_transportation_rate),2))) * 0.05
            self.total_payable_amount   = flt(flt(self.total_item_rate) + flt(self.total_transportation_rate),2)+flt(self.gst_amount,2)
