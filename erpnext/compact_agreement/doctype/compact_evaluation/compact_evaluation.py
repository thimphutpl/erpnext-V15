...
class CompactEvaluation(Document):
    ...
    def validate_achieved(self):
        ...
        if not hasattr(self, 'table_qwvz') or not self.table_qwvz:
            frappe.throw("Please add items with achieved in the table")
            return
            
        total_achieved = sum(flt(item.achieved or 0) for item in self.table_qwvz)
        
        if flt(total_achieved, 2) > 100.0:  # Strict check for > 100%
            frappe.throw(
    ...
    def before_save(self):
        """Calculate totals for 'achieved' and store as percent."""
        total_weightage = 0
        total_achieved = 0

        # Calculate totals from child table
        for item in self.table_qwvz:
            total_weightage += flt(item.weightage)
            total_achieved += flt(item.achieved)

        # Calculate percentages
        self.total_weighted_percent = total_weightage
        self.total_achieved_percent = total_achieved

        # Show success message
        frappe.msgprint("Totals calculated successfully: Weighted = {:.2f}%, Achieved = {:.2f}%".format(
            total_weightage, total_achieved))
