...
class MechanicalPayment(AccountsController):
	...
	def get_transactions(self):
		...
		if self.customer:
			...
			params.append(self.customer)

		if self.supplier:
			if self.payment_for == "Job Cards":
				conditions.append("customer = %s")
			else:
		...
		# transactions = frappe.db.sql("select name, outstanding_amount, customer from `tab{0}` where customer = '{1}' and branch = '{2}' and supplier = '{3}' and outstanding_amount > 0 and docstatus = 1 order by creation".format(self.payment_for, self.customer, self.branch, self.supplier), as_dict=1)
...
# 		target.transaction_type = "Mechanical Payment"
...
# 	doc = get_mapped_doc("Mechanical Payment", source_name, {
# 			"Mechanical Payment": {
