...
class MechanicalPayment(AccountsController):
    ...
    def make_gl_entry(self):
        ...
        party_type,party="",""
        if frappe.get_value("Account", self.income_account, "account_type") in ("Payable", "Receivable"):
            # party_type='Customer' if frappe.get_value("Account", self.income_account, "account_type") == 'Receivable' else 'Supplier'
            party_type='Customer'
            party=self.customer
        gl_entries = []
        ...
        for a in self.items:
            ...
            doc = frappe.get_doc(a.reference_type, a.reference_name)
            if a.reference_type == "Job Cards" or a.reference_type == "Fabrication And Bailey Bridge":
                if doc.jv:
