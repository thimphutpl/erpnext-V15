...
# #     #     self.net_amount = net_total-deduction_amount
# #     #     self.receivable_amount = net_total
# #     def validate_gst(self):
# #         """Calculate net amount and receivable amount including GST"""

# #         deduction_amount = sum(d.amount for d in (self.deducts or []))
        
# #         gst_amount = sum(item.gst_amount or 0 for item in (self.items or []))
...
# #         self.net_amount = net_total - deduction_amount-self.tds_amount
# #         self.receivable_amount = net_total
...
# #             gl_entries.append(
# #                 self.get_gl_dict({"account": receivable_account,
# #                                     "credit": flt(self.receivable_amount)+flt(self.gst_amount),
# #                                     "credit_in_account_currency": flt(self.receivable_amount)+flt(self.gst_amount),
# #                                     "cost_center": self.cost_center,
# #                                     "party_check": 1,
# #                                     "party_type": "Customer",
# #                                     "party": a.customer if a.customer else self.customer,
...
#         self.cost_center = get_branch_cc(self.branch)
#         sub_net = 0
#         sub_net = flt(self.deduction_amount) + flt(self.tds_amount)
#         if sub_net > 0:
#             self.net_amount = flt(self.receivable_amount) - sub_net
#         else:
#             self.net_amount = flt(self.receivable_amount)
...
#             doc = frappe.get_doc(a.reference_type, a.reference_name)
#             if cancel:
#                 amount = flt(doc.outstanding_amount + a.allocated_amount,2)
#             else:
#                 amount = flt(doc.outstanding_amount - a.allocated_amount,2)

#             if a.reference_type in ("Job Cards", "Fabrication And Bailey Bridge"):
#                 payable_amount = doc.total_amount+gst_amount
#                 # frappe.throw(str(payable_amount))
#             elif a.reference_type in ("Hire Charge Invoice"):
#                 payable_amount = doc.balance_amount + gst_amount
#                 # frappe.throw(str(payable_amount))
#             else:
#                 payable_amount = doc.balance_amount
...
#         child_map = {
#             "Job Card": "Job Card Item",
#             "Fabrication And Bailey Bridge": "Job Cards Item"
#         }
...
#             if use_child_gst:
#                 child_doctype = child_map.get(self.payment_for)
#                 # frappe.throw(str(child_doctype))
...
class MechanicalPayment(AccountsController):
    ...
    def make_gl_entry(self):
        ...
        if self.account_head and flt(self.gst_amount) > 0 and self.apply_gst==1:
            gl_entries.append(
                self.get_gl_dict({"account": self.account_head,
                                "debit": flt(self.gst_amount),
                                "debit_in_account_currency": flt(self.gst_amount),
        ...
        if self.items: 
            # frappe.throw(str(self.gst_amount))
            for a in self.items:
                against_voucher, against_voucher_type = None, None
                doc = frappe.get_doc(a.reference_type, a.reference_name)
                if a.reference_type == "Job Cards":
