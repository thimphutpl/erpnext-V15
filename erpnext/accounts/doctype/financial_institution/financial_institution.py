...
class FinancialInstitution(Document):
	...
	def get_branches(self):
		return frappe.get_all("Financial Institution Item", "*", {"parent": self.name}, order_by="branch_name")
	...
	def sync_branches(self):
		for item in self.items:
			...
			# else:
			branch = frappe.new_doc("Financial Institution Item")

			branch.update({
				"parent": self.name,
				"parenttype":"Finanical Institution",
