...
frappe.ui.form.on("Project Payment", {
	...
	get_invoices_without_gst: function (frm, cdt, cdn) {
		get_invoice_list_without_gst(frm);
		//assign_items(frm, cdt, cdn);
	},
			...
			callback: function (r) {
				if (r.message) {
					...
					r.message.forEach(function (inv) {
						...
						row.total_amount = flt(inv['total_gst_amount']);
						row.taxes_and_charges = inv['taxes_and_charges'];
						row.account_head = inv['acount_head'];
						row.tax_rate = flt(inv['tax_rate']);
						row.gst_amount = flt(inv['gst_amount'])
						row.total_gst_amount = flt(inv['total_gst_amount']);
...
var get_invoice_list_without_gst = function (frm) {
	if (frm.doc.project && frm.doc.party_type && frm.doc.party) {
		frappe.call({
			method: "erpnext.accounts.doctype.project_payment.project_payment.get_invoice_list_without_gst",
			args: {
				"project": frm.doc.project,
				"party_type": frm.doc.party_type,
				"party": frm.doc.party,
