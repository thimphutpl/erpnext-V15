...
from erpnext.accounts.utils import get_account_currency, get_balance_on, get_outstanding_invoices,check_clearance_date, cancel_exchange_gain_loss_journal
...
class PaymentEntry(AccountsController):
	...
	def on_submit(self):
		...
		self.adjust_outstanding_amount_for_emi_sale()
	...
	def adjust_outstanding_amount_for_emi_sale(self):
		if (self.references[0].reference_doctype == 'EMI Sales' or self.references[0].reference_doctype == 'Sales Invoice') and flt(self.references[0].outstanding_amount)>0:
			...
			if self.references[0].reference_doctype == "EMI Sales":
				...
				emi_sale_previous = frappe.db.get_value("EMI Sales", reference_no, "outstanding_amount", as_dict=1)
				emi_sale_outstanding = flt(emi_sale_previous.outstanding_amount)-flt(amount) if self.docstatus == 1 else flt(emi_sale_previous.outstanding_amount)+flt(amount)
				emi_sale = frappe.get_doc("EMI Sales",reference_no)
				emi_sale.db_set("outstanding_amount",emi_sale_outstanding)
				emi_sale.set_status(update=true)
				frappe.db.commit()
				...
				if ref_doc.salary_component and ref_doc.salary_structure and ref_doc.sales_order_type in ("Cost Sharing Installment", "Employee Installment"):
					...
					for d in ssl.deductions:
						if d.salary_component == ref_doc.salary_component and d.reference_number == item.reference_name and d.total_outstanding_amount > 0 and d.total_outstanding_amount == item.allocated_amount:
							if self.docstatus == 2:
								d.total_deducted_amount = flt(d.total_deducted_amount) - item.allocated_amount
								d.total_outstanding_amount = flt(d.total_outstanding_amount) + flt(item.allocated_amount)
							elif self.docstatus == 1:
								d.to_date = self.posting_date
								d.total_deducted_amount = flt(d.total_deducted_amount) + item.allocated_amount
								d.total_outstanding_amount = flt(d.total_outstanding_amount) - flt(item.allocated_amount)
					ssl.save(ignore_permissions=True)
					# nofity_hr(item.reference_doctype,item.reference_name,True,self.posting_date)
	...
	def get_current_tax_amount(self, tax):
		...
		if tax.charge_type == "Actual":
			...
			current_tax_amount = ((tax_rate / 100.0) if tax_rate else 1) * self.paid_amount_after_tax
		elif tax.charge_type == "On Previous Row Amount":
			current_tax_amount = ((tax_rate / 100.0) if tax_rate else 1) * self.get("taxes")[cint(tax.row_id) - 1].tax_amount

		elif tax.charge_type == "On Previous Row Total":
			current_tax_amount = ((tax_rate / 100.0) if tax_rate else 1) * self.get("taxes")[cint(tax.row_id) - 1].total
	...
	def get_current_tax_fraction(self, tax):
		...
		if cint(tax.included_in_paid_amount):
			...
			if tax.charge_type == "On Paid Amount":
				current_tax_fraction = (tax_rate / 100.0) if tax_rate else 1
			elif tax.charge_type == "On Previous Row Amount":
				current_tax_fraction = ((tax_rate / 100.0) if tax_rate else 1) * self.get("taxes")[
					cint(tax.row_id) - 1
...
# 	if dt == 'Sales Order' and doc.c2_status:
# 		total = 0
# 		c2 = frappe.get_doc('C2 Status', doc.c2_status)
# 		for i in c2.table_yefo:
# 			total += i.advances_paid
# 		paid_amount = total
...
# 						"bill_no": doc.get("bill_no"),
# 						"due_date": doc.get("due_date"),
# 						"total_amount": paid_amount if dt == 'Sales Order' and doc.c2_status else grand_total,
# 						"outstanding_amount": paid_amount if dt == 'Sales Order' and doc.c2_status else outstanding_amount,
# 						"allocated_amount": paid_amount if dt == 'Sales Order' and doc.c2_status else outstanding_amount,
# 					},
# 				)
# 	pe.setup_party_account_field()
# 	pe.set_missing_values()
...
def get_permission_query_conditions(user):
	...
	if user == "Administrator" or "System Manager" in user_roles or "Account Manager" in user_roles: 
		return
