...
def get_data(query):
	...
	for d in datas:
		...
		# Skip if amount is zero AND document is not cancelled
		if flt(d.amount) == 0 and d.cheque_status != 'CANCELLED':
			continue
			
		row = [d.voucher_type, d.voucher_no, d.voucher_date, d.cheque_no, d.cheque_date, 
		       d.amount, total[key], d.recipient, d.cheque_status, d.cancelled_amount]
		data.append(row)
		total_amount += flt(d.amount)
		total_cancelled += flt(d.cancelled_amount)
...
def construct_query(filters=None):
	query = """SELECT 'Journal Entry' as voucher_type, je.name as voucher_no, je.posting_date as voucher_date, 
		...
		AND pe.posting_date BETWEEN '{from_date}' AND '{to_date}' 
		AND NOT EXISTS (SELECT 1 from `tabPayment Entry` pe1 where pe1.amended_from = pe.name)
		AND (CASE WHEN pe.base_paid_amount > 0 THEN pe.base_paid_amount ELSE pe.base_received_amount END) != 0
		""".format(from_date=filters.from_date, to_date=filters.to_date)
	return query
