...
class BudgetProposal(Document):
    ...
    def auto_fill_approved_budget(self):
        """Auto-fill approved_budget with initial_budget for rows not manually changed."""
        for row in self.get("accounts"):
            # Skip rows where user manually changed approved_budget
            if row.get("approved_budget_manually_changed"):
                continue

            # Auto-fill only for new/empty rows
            if not row.approved_budget:
                row.approved_budget = row.initial_budget or 0
