...
# # 	# 		args.budget_activity = a.from_budget_activity  # Assuming field name is `from_budget_activity`
# # 	# 		args.budget_sub_activity = a.from_budget_sub_activity  # Assuming field name is `from_budget_sub_activity`
# # 	# 		args.source_of_fund = a.source_of_fund  # Ensure this field exists in the child table
...
# # 		args.cost_center = self.from_cost_center if self.budget_against == "Cost Center" else None
# # 		args.project = self.from_project if self.budget_against == "Project" else None
# # 		args.fiscal_year = self.fiscal_year  # Keeps fiscal year as 2025-26 for reference
# # 		args.posting_date = self.appropriation_on
# # 		args.company = self.company
		
# # 		# Extract the start year from fiscal_year (e.g., "2025-26" → 2025)
# # 		try:
...
# # 				month = datetime.date(2023, month_id, 1).strftime("%B")
# # 				if a.from_month == month:
# # 					month_num = str(month_id).zfill(2)  # Ensures 2-digit month (01, 02, ...)
# # 					first_day = f"{fiscal_start_year}-{month_num}-01"  # Proper YYYY-MM-DD format
...
# # 			args.account = a.from_account
# # 			args.amount = a.amount
# # 			args.posting_date = first_day  # Now a valid date string (e.g., "2025-08-01")
...
# 				if not from_budget_exist:
# 					frappe.throw(_(
# 						"Budget Release record does not exist against {0} '{1}' and account '{2}' for fiscal year {3}"
# 					).format(self.budget_against, from_budget_against, d.from_account, self.fiscal_year))
...
# 						"""
# 						select
# 							ba.name, ba.account from `tabBudget Release` b, `tabBudget Release Account` ba
# 						where
# 							ba.parent = b.name and b.docstatus < 2 and b.company = %s and %s=%s and
# 							b.fiscal_year=%s and ba.account =%s """
# 						% ("%s", budget_against_field, "%s", "%s", "%s"),
...
# 				monthly_budget_check = frappe.db.get_single_value("Budget Settings","monthly_budget_check")
# 				if from_account:
# 					from_budget_account = frappe.get_doc("Budget Release Account", from_account[0].name)
# 					total = flt(from_budget_account.budget_amount) - flt(d.amount)
# 					budget_sent = flt(from_budget_account.budget_sent) + flt(d.amount)
# 					if cancel:
# 						total = flt(from_budget_account.budget_amount) + flt(d.amount)
...
# 					)
# 				if to_account:
# 					to_budget_account = frappe.get_doc("Budget Release Account", to_account[0].name)
# 					total = flt(to_budget_account.budget_amount) + flt(d.amount)
# 					budget_received = flt(to_budget_account.budget_received) + flt(d.amount)
# 					if cancel:
# 						total = flt(to_budget_account.budget_amount) - flt(d.amount)
...
class BudgetReappropiation(Document):
	...
	# 		budget_against_field = frappe.scrub(self.budget_against)
	# 		from_budget_against = self.from_cost_center if self.budget_against == "Cost Center" else self.from_project
	# 		# frappe.throw(str(from_budget_against))
	# 		to_budget_against = self.to_cost_center if self.budget_against == "Cost Center" else self.to_project
	# 		# frappe.throw(str(to_budget_against))
	# 		for d in self.items:
	# 			from_month = d.from_month
	# 			to_month = d.to_month
	# 			if d.amount <= 0:
	...
	# 				AND b.fiscal_year = %s
	# 				AND ba.account = %s
	# 			""", (self.company, self.from_cost_center, self.fiscal_year, d.from_account), as_dict=1)
	# 			# frappe.throw(frappe.as_json(from_account))
	...
	# 			(self.company, to_budget_against, self.fiscal_year, d.to_account),
	# 			as_dict=1)
	# 			frappe.throw(frappe.as_json(to_account))
