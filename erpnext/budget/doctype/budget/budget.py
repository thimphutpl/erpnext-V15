...
#             else:
#                 if args.project or args.employee_group == "Teachers (RBA)":
#                     condition = " and b.project = '{}'".format(args.project)
#                     budget_cost_center = args.cost_center
#                     frappe.log_error(
#                         title="Budget Cost Center Debug",
#                         message=f"budget_cost_center: {budget_cost_center}\nargs: {args}"
#                     )
#                 else:
...
#                     if bud_acc_dtl.is_centralized_budget and args.employee_group != "Teachers (RBA)":
...
#                 as_dict=True
#             )
#             frappe.log_error(str(budget_records))
...
def validate_expense_against_budget(args, throw_error=True):
    ...
    budget_cost_center = None
    budget_found = False  # Flag to track if budget was found
        ...
        if (
            ...
            if budget_against == "company":
                ...
                if args.project or args.employee_group == "Teachers (RBA)":
                    ...
                    if frappe.db.get_value("Account", args.account, "is_centralized_budget") == 1 and frappe.db.get_value("Account", args.account, "cost_center") and args.employee_group != "Teachers (RBA)":
                        args.cost_center = frappe.db.get_value("Account", args.account, "cost_center")
                    bud_acc_dtl = frappe.get_doc("Account", args.account)
                    if bud_acc_dtl.is_centralized_budget:
                        budget_cost_center = bud_acc_dtl.cost_center
                    elif frappe.db.get_value("Cost Center", args.cost_center, "use_budget_from_head_quarter_cost_center") == 1 and args.employee_group == "Teachers (RBA)":	
                        cc_doc = frappe.get_doc("Cost Center", args.cost_center)
                        budget_cost_center = cc_doc.head_quarter_cost_center if cc_doc.head_quarter_cost_center else args.cost_center
                    else:
                        cc_doc = frappe.get_doc("Cost Center", args.cost_center)
            ...
            # ===== LOG WHAT WAS FOUND =====
            frappe.log_error(
                title=f"Budget Check - {budget_cost_center}",
                message=f"""
                Cost Center: {budget_cost_center}
                Activity: {args.budget_activity}
                Sub Activity: {args.budget_sub_activity}
                Records Found: {len(budget_records) if budget_records else 0}
                Records: {str(budget_records)}
                """
            )

            if budget_records:
                # ===== FOUND BUDGET - Validate and STOP =====
                validate_budget_records(args, error, budget_records, throw_error)
                budget_found = True
                break  # <-- CRITICAL: Stop checking other cost centers
            else:
                # ===== NO BUDGET FOUND for this cost center =====
                # For Teachers (RBA), we should have found it in MTC
                # If not, continue to next iteration (but this shouldn't happen)
                frappe.log_error(
                    title=f"No Budget Found - {budget_cost_center}",
                    message=f"No budget found for {budget_cost_center} with activity {args.budget_activity}"
                )
                continue

    # ===== AFTER THE LOOP - Only throw if no budget found =====
    if not budget_found:
        error_msg = "Budget not available for <b>%s </b> in %s <b>%s</b> for Activity: <b>%s</b> Sub Activity: <b>%s</b> Source of Fund: <b>%s</b> for fiscal year <b>%s</b>" % (
            args.account, 
            args.budget_against_field, 
            args.cost_center,
            args.budget_activity, 
        ...
        frappe.log_error(
            title="Budget Not Found - Final",
            message=f"No budget found in any cost center.\nError: {error_msg}\nArgs: {args}"
        )
        ...
        if (
            ...
            if budget_against == "company":
                ...
                if args.project:
                    ...
                    # ===== STORE ORIGINAL COST CENTER BEFORE MODIFICATION =====
                    original_cost_center = args.cost_center
                    
                    # ===== CHECK IF TEACHERS (RBA) BASED ON ORIGINAL COST CENTER =====
                    teachers_rba_cost_centers = [
                        "001 - RBA HEAD QUARTER - RBA",
                        "005 - MTC, TENCHOLING - RBA",
                        "008 - WING 9, GELEPHU - RBA",
                        "010 - WING 11, DAMTHANG - RBA"
                    ]
                    is_teachers_rba_cost_center = original_cost_center in teachers_rba_cost_centers

                    # Account name for PF contribution
                    pf_account_name = ["24.03 - Contributions - Provident Fund - RBA", "1.01 - Pay & Allowances - RBA"]
                    is_pf_account = args.account == pf_account_name

                    # ===== ONLY FOR PF ACCOUNT + TEACHERS RBA COST CENTER =====
                    is_teachers_rba_pf = is_pf_account and is_teachers_rba_cost_center

                    # ===== CRITICAL FIX: Check PF + Teachers (RBA) FIRST =====
                    if is_teachers_rba_pf:
                        # For PF + Teachers (RBA), use the original cost center directly
                        budget_cost_center = original_cost_center
                    else:
                        # ===== FOR ALL OTHER ACCOUNTS: Use centralized budget logic =====
                        # Check if account has centralized budget
                        if frappe.db.get_value("Account", args.account, "is_centralized_budget") == 1 and frappe.db.get_value("Account", args.account, "cost_center"):
                            args.cost_center = frappe.db.get_value("Account", args.account, "cost_center")
                        ...
                        if bud_acc_dtl.is_centralized_budget:
                            # Use centralized budget cost center (HQ)
                            budget_cost_center = bud_acc_dtl.cost_center
                        elif frappe.db.get_value("Cost Center", args.cost_center, "use_budget_from_head_quarter_cost_center") == 1:
                            cc_doc = frappe.get_doc("Cost Center", args.cost_center)
                            budget_cost_center = cc_doc.head_quarter_cost_center if cc_doc.head_quarter_cost_center else args.cost_center
                ...
                # ===== BUILD CONDITION WITH CORRECT COST CENTER =====
                condition = " and b.cost_center='{}'".format(budget_cost_center)
            
            args.is_tree = False
            if not args.project:
                args.committed_cost_center = args.cost_center
                # ===== SET args.cost_center FOR THE BUDGET CHECK =====
                args.cost_center = budget_cost_center
