	...
	for d in datas:
		...
		if filters.monthly_budget:
			...
			committed = frappe.db.sql(
				"select SUM(amount) from `tabCommitted Budget` where cost_center = %s and account = %s and reference_date BETWEEN %s and %s", (d.cost_center, d.account, filters.from_date, filters.to_date))[0][0]
			# frappe.msgprint(str(d.cost_center))
