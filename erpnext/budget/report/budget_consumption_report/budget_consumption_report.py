...
def execute(filters=None):
    ...
    if filters.monthly_budget:
        if filters.month:
            ...
            fy_start = getdate(filters.year_start_date)
            ...
            # Calculate the correct year for the selected month
            year = fy_start.year

            # If the fiscal year doesn't start in January,
            # months before the fiscal start belong to the next calendar year.
            if fy_start.month != 1 and month_id < fy_start.month:
                year += 1

            from_date = getdate(f"{year}-{month_id:02d}-01")
            to_date = get_last_day(from_date)
...
def get_extra_entries(filters, from_date, to_date):
    ...
    committed_query = """
        SELECT DISTINCT account, cost_center, project, budget_activity, 
               budget_sub_activity, source_of_fund, company
        ...
        if d.budget_amount > 0:
            if filters.budget_against != "Project":
                ...
                row = {
                    ...
                    "financing_code": d.source_of_fund,

                    "financing_title": frappe.db.get_value(
                        "Source of Fund",
                        d.source_of_fund,
                        "source_of_fund"
                    ) or d.source_of_fund,

                    "account_code": d.account.split(" ")[0]
                        if d.account else "",

                    "account_title": d.account.replace(
                        d.account.split(" ")[0],
                        ""
                    ).strip() if d.account else "",
                    ...
                    "budget_received": adjustment,

                    "budget_amount": current,
...
def construct_query(from_date, to_date, filters=None):
    ...
    if filters.monthly_budget and filters.month:
        ...
        query = """select b.cost_center, ba.account, b.project, ba.budget_activity, ba.budget_sub_activity, ba.source_of_fund,
            ...
            SUM(ba.supplementary_budget) as supplement,
            COALESCE((
                SELECT SUM(bra.released_budget)
                FROM `tabBudget Release` br
                INNER JOIN `tabBudget Release Account` bra ON bra.parent = br.name
        ...
        """.format(
            ...
            condition=condition, 
            month_name=month_field_name.lower(),
            from_date=from_date,
            to_date=to_date
        )
        ...
        query = """select b.cost_center, ba.account, b.project, ba.budget_activity, ba.budget_sub_activity, ba.source_of_fund,
            ...
            COALESCE((
                ...
                WHERE br.docstatus = 1 
                    ...
                    AND bra.source_of_fund = ba.source_of_fund
                    AND br.posting_date BETWEEN '{from_date}' AND '{to_date}'
            ), 0) as release_deducted,
            COALESCE((
                SELECT SUM(bra.supplementary_budget)
                FROM `tabBudget Release` br
                INNER JOIN `tabBudget Release Account` bra ON bra.parent = br.name
                WHERE br.docstatus = 1 
                    AND br.fiscal_year = "{fiscal_year}"
