...
class StockController(AccountsController):
	...
	def get_price_costing(self, so_detail, item_code):
		price_costing_name = frappe.db.get_value("Sales Order Item", so_detail, "price_template")
		price_costing = frappe.db.sql(
			"""
			select
				pci.landed_cost
			from
				`tabPrice Costing` pc, `tabPrice Costing Item` pci			
			where
				pc.name = pci.parent and pc.name=%s and pci.item=%s LIMIT 1
		""",
			(price_costing_name, item_code),
			as_dict=True,
		)
		return price_costing
		...
		sl_dict = frappe._dict(
			{
				...
				"make": d.get("make", None),
				"transmission_type_manualautomatic": d.get("transmission_type_manualautomatic", None),
				"model": d.get("model", None),
				"engine_cc": d.get("engine_cc", None),
				"model_year": d.get("model_year", None),
				"fuel_type": d.get("fuel_type", None),
				"seating_capacity": d.get("seating_capacity", None),
				"vehicle_color": d.get("vehicle_color", None),
				"gvw_tonnage": d.get("gvw_tonnage", None),
			}
		)
