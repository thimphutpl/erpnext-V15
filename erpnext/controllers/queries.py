...
def price_template_list(doctype, txt, searchfield, start, page_len, filters):
    ...
    if selling_uom == '':
        cond += "IF(b.selling_uom IS NULL,'',b.selling_uom) = '' and "
    elif selling_uom != '':
        check_loc = frappe.db.sql(
            "select 1 from `tabSelling Price` sp, `tabSelling Price Rate` spr where spr.parent = sp.name and spr.particular='{0}' and spr.selling_uom='{1}' and sp.to_date >= '{2}'".format(
                filters.get("item_code"), selling_uom, nowdate()
            )
        )
        if not check_loc:
            cond += "IF(b.selling_uom IS NULL,'',b.selling_uom) = '' and "
        else:
            cond += "IF(b.selling_uom IS NULL,'',b.selling_uom) = '{}' and ".format(selling_uom)
    ...
    query = """
        ...
        """.format(
        filters.get("branch"), filters.get("item_code"), cond, filters.get("transaction_date")
    )
    ...
    if not item_price:
        ...
        if not item_species:
            ...
            item_price = frappe.db.sql(
                ...
                group by a.parent
            """.format(
                    filters.get("branch"), timber_class, timber_type, item_sub_group, cond, filters.get("transaction_date")
                )
            )

    if not item_price:
        frappe.throw(
            "Rate for Item: <b> '{0}' </b> Is Not Defined In Selling Price List, Please Define The Rate".format(
...
def customer_price_template_list(doctype, txt, searchfield, start, page_len, filters):
    ...
    if filters.get("location"):
        ...
        item_price = frappe.db.sql(
            """ select a.name, b.particular, b.selling_price 
                            ...
                            where a.name = b.parent 
                                ...
                                and a.customer = '{4}'
                            """.format(
                filters.get("branch"), cond, filters.get("item_code"), filters.get("transaction_date"), filters.get("customer")
            )
        )
    if not item_price:
        item_price = frappe.db.sql(
    ...
    if not item_price:
        ...
        if not item_species:
            ...
            item_sub_group = frappe.db.get_value("Item", filters.get("item_code"), "item_sub_group")
            item_price = frappe.db.sql(
                """ select a.name, b.particular, b.timber_type, b.selling_price  
                                from `tabCustomer Selling Price` a, `tabSelling Price Rate` b 
                                where a.name = b.parent 
                                and a.branch = '{0}' 
                                {1}
    ...
    if not item_price:
        frappe.throw(
            "Rate for Item: <b> '{0}' </b> Is Not Defined In Customer Selling Price List, Please Define The Rate".format(
                filters.get("item_code")
            )
        )
    return item_price
...
def get_costing_component(doctype, txt, searchfield, start, page_len, filters):
    ...
    return frappe.db.sql(
        "select name, particular from `tabCosting Component` where '{0}' >= from_date and '{1}' <= to_date and branch = '{2}'".format(
            filters.get("from_date"), filters.get("to_date"), filters.get("branch")
        )
    )
...
# NRDCL/BTF common custom functions
@frappe.whitelist()
def filter_branch_wh(doctype, txt, searchfield, start, page_len, filters):
    if not filters.get("branch"):
        frappe.throw("Select Branch First")
    return frappe.db.sql(
        "select a.parent from `tabWarehouse Branch` a, tabWarehouse b where a.parent = b.name and a.branch = %s and b.disabled = 0",
        filters.get("branch"),
    )
...
def filter_branch_rng(doctype, txt, searchfield, start, page_len, filters):
    if not filters.get("branch"):
        frappe.throw("Select Branch First")
    return frappe.db.sql(
        "select distinct a.parent from `tabRange Location` a, tabRange b where a.parent = b.name and a.branch = '{0}' and b.is_disabled = 0".format(
            filters.get("branch")
        )
    )
...
def filter_range_location(doctype, txt, searchfield, start, page_len, filters):
    ...
    return frappe.db.sql(
        "select a.location from `tabRange Location` a, tabRange b where a.parent = b.name and b.name= '{0}' and b.is_disabled = 0".format(
            filters.get("range")
        )
    )
...
def get_cop_list(doctype, txt, searchfield, start, page_len, filters):
    ...
    return frappe.db.sql(
        "select a.parent, b.item_sub_group, b.cop_amount from `tabCOP Branch` a, `tabCOP Rate Item` b where a.parent = b.parent and a.branch = %s and b.item_sub_group = %s and exists (select 1 from `tabCost of Production` where name = a.parent and %s between from_date and to_date)",
        (filters.get("branch"), str(item_sub_group), filters.get("posting_date")),
    )
...
def get_measurements(doctype=None, txt=None, searchfield=None, start=None, page_len=None, filters=None):
    ...
    li = frappe.db.sql(
        """select isgm.material_measurement
    ...
    """.format(
            filters.get("item_sub_group"), cond
        )
    )
...
def get_crm_users(doctype, txt, searchfield, start, page_len, filters):
    return frappe.db.sql(
        """select name, full_name from `tabUser` where account_type = 'CRM' and enabled = 1
                and ({key} like %(txt)s
