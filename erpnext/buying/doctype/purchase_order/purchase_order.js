...
frappe.ui.form.on("Purchase Order", {
	setup: function (frm) {
		...
		// ✅ FIXED: Filter branch using custom query method
	...
	company: function (frm) {
		...
		// Reset branch and cost_center when company changes
	...
	branch: function (frm) {
		if (frm.doc.branch) {
			// Get the cost_center from the selected branch
			frappe.call({
				...
				callback: function (r) {
					if (r.message && r.message.cost_center) {
						// Check if the cost center is non-group
						frappe.call({
							method: "frappe.client.get_value",
							args: {
								doctype: "Cost Center",
	...
	// ✅ Updated: When cost_center changes, update all child rows
	cost_center: function (frm) {
		...
		// Optional: Auto-populate warehouse from cost_center
		frappe.call({
			method: "frappe.client.get_value",
			args: {
				doctype: "Cost Center",
...
// ✅ Updated: When adding new item rows, copy parent cost_center to child
frappe.ui.form.on("Purchase Order Item", {
	items_add: function(frm, cdt, cdn) {
		var row = frappe.get_doc(cdt, cdn);
		
		// Copy cost_center from parent to new item row
		if (frm.doc.cost_center) {
			row.cost_center = frm.doc.cost_center;
			refresh_field("cost_center", cdn, "items");
		}
		
		// Copy schedule_date from parent to new item row
		if (frm.doc.schedule_date) {
