...
class CustomWorkflow:
    def __init__(self, doc):
        ...
        if self.doc.doctype == "Material Request":
            ...
            self.reports_to	= frappe.db.get_value("Employee", frappe.db.get_value("Employee", {'user_id':self.employee[0]}, "reports_to"), self.field_list)
    ...
    def set_approver(self, approver_type):
        if approver_type == "Supervisor":
            if self.doc.doctype in ("Travel Request","Travel Authorization", "Travel Claim","Employee Separation","Vehicle Request",  "Repair And Services","Overtime Application"):
            ...
            if not procurement_head:
                frappe.throw("procurement head not set in buying setting")
            self.reports_to = frappe.db.get_value("Employee", {"name":procurement_head}, self.field_list)
            ...
            vars(self.doc)[self.doc_approver[2]] = officiating[2] if officiating else self.reports_to[2]
            vars(self.doc)[self.doc_approver[3]] = officiating[2] if officiating else self.reports_to[0]
        elif approver_type =="AFD Head Approval":
            afd_head=frappe.db.get_single_value("Buying settings","afd_head_approval")
            if not afd_head:
                frappe.throw("AFD head not set in buying setting")
            self.reports_to = frappe.db.get_value("Employee", {"name":afd_head}, self.field_list)
...
def get_field_map():
    return {
        ...
        "Material Request": ["approver","approver_name","approver_designation","approver_email"],
...
def validate_compact_agreement(doc, method):
    if doc.workflow_state == "Pending Head of Company Approval":
        if not doc.head_of_company:
            frappe.throw("Head of Company is not assigned.")
    
    elif doc.workflow_state == "Pending Head of OPM Approval":
        if not doc.head_of_opm:
            frappe.throw("Head of OPM is not assigned.")
    
    elif doc.workflow_state == "Approved":
        frappe.msgprint(f"Compact Agreement {doc.name} has been approved.")
    
    elif doc.workflow_state == "Cancelled":
        frappe.msgprint(f"Compact Agreement {doc.name} has been cancelled.")
...
doc_events = {
    "Compact Agreement": {
        "validate": validate_compact_agreement,
        "on_update": on_update,
    }
}