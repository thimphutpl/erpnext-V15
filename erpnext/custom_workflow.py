...
class CustomWorkflow:
	def __init__(self, doc):
		...
		if self.doc.doctype != "Material Request" and self.doc.doctype not in ("Asset Issue Details", "Compile Budget","POL Expense", "Repair And Services", "Asset Movement", "Budget Reappropiation", "Employee Advance","Vehicle Request"):
			...
			self.gm_approver	= frappe.db.get_value("Employee", frappe.db.get_single_value("Accounts Settings", "approver_name"), self.field_list)
			self.adm_section_manager =frappe.db.get_value("Employee", frappe.db.get_single_value("HR Settings", "aa_approver"), self.field_list)
	...
	def set_approver(self, approver_type):
		...
		elif approver_type == "ADM Approver":
	...
	def vehicle_request(self):		
		if self.new_state.lower() in ("Draft".lower()):
			...
			self.set_approver("ADM Approver")
	...
	def travel_claim(self):
		...
		if self.new_state.lower() in ("Draft".lower()):
			...
			if frappe.session.user != self.doc.approver:
				frappe.throw("Only {approver} can Reject this Travel Claim.".format(approver=self.doc.approver))
		
		elif self.new_state.lower() == ("Cancelled".lower()):
			if frappe.session.user != self.doc.approver:
				frappe.throw("Only {approver} can Cancel this Travel Claim.".format(approver=self.doc.approver))
		else:
			frappe.throw(_("Invalid Workflow State {}").format(self.doc.workflow_state))
...
class NotifyCustomWorkflow:
	...
	def notify_employee(self):
		...
		parent_doc = frappe.get_doc(self.doc.doctype, self.doc.name)
		args = parent_doc.as_dict()
		new_state = getattr(self, "new_state", self.doc.workflow_state) 
		args["workflow_state"] = new_state
	...
	def notify_approver(self):
		if self.doc.get(self.doc_approver[0]):
			parent_doc = frappe.get_doc(self.doc.doctype, self.doc.name)
			args = parent_doc.as_dict()
			 # FIX: Explicitly set the new workflow state for the notification
			new_state = getattr(self, "new_state", None) or "Waiting Approval" 
			args["workflow_state"] = new_state
			if self.doc.doctype == "Leave Application":
				template = frappe.db.get_single_value('HR Settings', 'leave_application_approval_notification_template')
				if not template:
	...
	def notify_hr_users(self):
		...
		if hr_users:
			...
			if self.doc.doctype == "Leave Application":
				...
				template = frappe.db.get_single_value('HR Settings', 'travel_authorization_approval_notification_template')
				if not template:
					frappe.msgprint(_("Please set default template for Travel Authorization Approval Notification in HR Settings."))
					return
			elif self.doc.doctype == "Vehicle Request":
				template = frappe.db.get_single_value('HR Settings', 'vehicle_request_approval_notification_template')
				if not template:
			...
			frappe.msgprint("No HR Users in the Email Group")
	...
	def send_notification(self):
		...
		if (self.doc.doctype not in self.field_map) or not frappe.db.exists("Workflow", {"document_type": self.doc.doctype, "is_active": 1}):
			return
		if self.new_state == "Draft" or self.new_state == self.old_state:
			...
			elif self.doc.doctype in ("Employee Benefits","Employee Separation","Vehicle Request"):
...
def get_field_map():
	return {
		...
		"Vehicle Request": ["approver", "approver_name", "approver_designation"],
