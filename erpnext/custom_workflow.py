...
class CustomWorkflow:
	def __init__(self, doc):
		...
		if self.doc.doctype == "Asset Movement":
			...
			if department != "CHIEF EXECUTIVE OFFICE - STCBL":
		...
		if self.doc.doctype in ("SWS Application"):
			self.hrgm = frappe.db.get_value("Employee",frappe.db.get_single_value("HR Settings","sws_application_approver"), self.field_list)
		...
		if self.doc.doctype == "Employee Advance":
			...
			self.ceo	= frappe.db.get_value("Employee", frappe.db.get_single_value("HR Settings", "ceo"), self.field_list)
			...
			# self.reports_to	= frappe.db.get_value("Employee", frappe.db.get_value("Employee", {'user_id':self.doc.owner}, "reports_to"), self.field_list)
			# self.reports_to = frappe.db.get_value("Employee", {"name":frappe.db.get_value("Employee", self.doc.employee, "reports_to")}, self.field_list)
			if not frappe.db.exists("Department Approver",{"parent": frappe.db.get_value("Employee", self.doc.employee, "division"), "parentfield": "employee_advance_approver"}):
				frappe.throw(_("Employee Advance Approver is not set for the department"))
			self.department_ea_approver = frappe.db.get_value("Employee",{"user_id": frappe.db.get_value("Department Approver",{"parent": frappe.db.get_value("Employee", self.doc.employee, "division"), "parentfield": "employee_advance_approver"},"approver")},self.field_list)
			...
			department = frappe.db.get_value("Employee", self.doc.employee ,"division")
			if not department:
				frappe.throw("Set division for {}".format(self.doc.employee))
	...
	def set_approver(self, approver_type):
		if approver_type == "Supervisor":
			if self.doc.doctype in ("Employee Separation","Vehicle Request", "Material Request", "Repair And Services"):
		...
		elif approver_type =="Account Approver":
			officiating = get_officiating_employee(self.ac_approver[3])
			if officiating:
				officiating = frappe.db.get_value("Employee", officiating[0].officiate, self.field_list)
			vars(self.doc)[self.doc_approver[0]] = officiating[0] if officiating else self.ac_approver[0]
			vars(self.doc)[self.doc_approver[1]] = officiating[1] if officiating else self.ac_approver[1]
			vars(self.doc)[self.doc_approver[2]] = officiating[2] if officiating else self.ac_approver[2]
			...
			if self.doc.doctype in ("Employee Transfer"):
				officiating = get_officiating_employee(self.expense_approver[3])
				if officiating:
					officiating = frappe.db.get_value("Employee", officiating[0].officiate, self.field_list)
				vars(self.doc)[self.doc_approver[0]] = officiating[0] if officiating else self.expense_approver[0]
		...
		elif approver_type == "Department Approver":
			officiating = get_officiating_employee(self.department_ea_approver[3])
			if officiating:
				officiating = frappe.db.get_value("Employee", officiating[0].officiate, self.field_list)
			vars(self.doc)[self.doc_approver[0]] = officiating[0] if officiating else self.department_ea_approver[0]
			vars(self.doc)[self.doc_approver[1]] = officiating[1] if officiating else self.department_ea_approver[1]
			vars(self.doc)[self.doc_approver[2]] = officiating[2] if officiating else self.department_ea_approver[2]
	...
	def sws_application(self):
		if self.new_state.lower() in ("Draft".lower(), "Waiting Approval".lower()):
			self.set_approver("HRGM")
		if self.new_state.lower() == "Verified By Supervisor".lower():
	...
	def travel_authorization(self):
		...
		if self.new_state.lower() in ("Waiting Supervisor Approval".lower()):
			...
			# 	frappe.throw("Only {} can Apply this document".format(self.doc.verifier))
			self.set_approver("Supervisor")

		if self.new_state.lower() in ("Waiting for HOD Approval".lower()):
		...
		# if self.new_state.lower() in ("Approved".lower()):
			# if not self.doc.approver or self.doc.approver == "":
			# 	frappe.throw("Please set approver in Travel Authorization document.")
			# if self.doc.advance_approver_name != frappe.session.user:
			# 	frappe.throw("Only {} can Approve this document".format(self.doc.advance_approver_name))
			# if self.doc.verifier != frappe.session.user:
			# 	frappe.throw("Only {} can Approve this document".format(self.doc.verifier))
	...
	def employee_advance(self):
		...
		if self.new_state.lower() in ("Verified".lower()):
			# if self.doc.owner != frappe.session.user:
			# 	frappe.throw("Only {} can Apply this document".format(self.doc.owner))
			self.set_approver("Account Approver")
	...
	def imprest_advance(self):
		...
		elif self.new_state.lower() in ("Waiting for Submitting".lower(), "Rejected".lower()) and self.old_state.lower() == "Waiting Approval".lower():
		...
		elif self.new_state.lower() in ("Submitted".lower(), "Rejected".lower()) and self.old_state.lower() == "Waiting for Submitting".lower():
			if not "Imprest Manager" in frappe.get_roles(frappe.session.user):
...
class NotifyCustomWorkflow:
	def __init__(self,doc):
		...
		self.doc_approver	= self.field_map[self.doc.doctype]
		self.field_list		= ["user_id","employee_name","designation","name"]
		if self.doc.doctype not in ("Material Request","Asset Issue Details", "Project Capitalization", "POL Expense", "Prepare Audit Plan", "Imprest Advance", "Imprest Recoup", "Budget Reappropiation", "Supplementary Budget"):
			self.employee   = frappe.db.get_value("Employee", self.doc.employee, self.field_list)
	...
	def notify_employee(self):
		if self.doc.doctype not in ("Material Request","Asset Issue Details","Repair And Services","Project Capitalization","POL Expense","Imprest Advance","Imprest Recoup", "Budget Reappropiation", "Supplementary Budget"):
