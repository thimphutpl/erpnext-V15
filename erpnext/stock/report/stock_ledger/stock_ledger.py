...
def update_available_serial_nos(available_serial_nos, sle):
    ...
    key = (sle.item_code, sle.warehouse)
    if key not in available_serial_nos:
        # Convert to string before calling
        posting_date = sle.posting_date.strftime('%Y-%m-%d') if hasattr(sle.posting_date, 'strftime') else str(sle.posting_date)
        posting_time = sle.posting_time.strftime('%H:%M:%S') if hasattr(sle.posting_time, 'strftime') else str(sle.posting_time)
        
        stock_balance = get_stock_balance_for(
            sle.item_code, 
            sle.warehouse, 
            posting_date,  # Now it's definitely a string
            posting_time
        )
        serials = get_serial_nos(stock_balance["serial_nos"]) if stock_balance["serial_nos"] else []
        available_serial_nos.setdefault(key, serials)
