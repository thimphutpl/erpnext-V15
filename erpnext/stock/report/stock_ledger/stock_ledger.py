...
# 		#frappe.throw(str(sle.voucher_type))
...
# 		conditions.append("voucher_no=%(voucher_type)s")
...
def execute(filters=None):
    ...
    stock_entry_details = {}
    if any(sle.get('voucher_type') == 'Stock Entry' for sle in sl_entries):
        stock_entry_details = get_stock_entry_details(sl_entries, filters)
    ...
    for sle in sl_entries:
        ...
        # Override with actual values if it's a Stock Entry and we have details
        if sle.voucher_type == 'Stock Entry' and sle.voucher_no and stock_entry_details.get(sle.voucher_no):
            sle.update(stock_entry_details[sle.voucher_no])
...
def get_stock_entry_details(sl_entries, filters=None):
    """Get expiry_date and manufacture_date from Stock Entry doctype with filters"""
    
    # Get filter values
    stock_entry_type_filter = None
    if filters and filters.get('stock_entry_type'):
        stock_entry_type_filter = filters.get('stock_entry_type')
        if not isinstance(stock_entry_type_filter, list):
            stock_entry_type_filter = [stock_entry_type_filter]
    
    # Get unique Stock Entry vouchers from sl_entries
    stock_entries = list(set([sle.get('voucher_no') for sle in sl_entries 
                             if sle.get('voucher_type') == 'Stock Entry' and sle.get('voucher_no')]))
    
    if not stock_entries:
        return {}
    
    # Build filters for Stock Entry query
    stock_entry_filters = {"name": ["in", stock_entries]}
    
    # Add stock_entry_type filter if provided
    if stock_entry_type_filter:
        stock_entry_filters["stock_entry_type"] = ["in", stock_entry_type_filter]
    
    # Get fields from Stock Entry doctype
    entries = frappe.get_all(
        "Stock Entry",
    ...
    for entry in entries:
        result[entry.name] = {
            ...
            "stock_entry_purpose": entry.purpose
        }
...
def get_stock_ledger_entries(filters, items):
    ...
    # Apply voucher_type filter (works directly on SLE)
    if filters.get("voucher_type"):
        voucher_types = filters.get("voucher_type")
        if not isinstance(voucher_types, list):
            voucher_types = [voucher_types]
        query = query.where(sle.voucher_type.isin(voucher_types))

    # Handle stock_entry_type filter (needs join with Stock Entry)
    if filters.get("stock_entry_type"):
        se = frappe.qb.DocType("Stock Entry")
        stock_entry_types = filters.get("stock_entry_type")
        if not isinstance(stock_entry_types, list):
            stock_entry_types = [stock_entry_types]
        
        # Get filtered stock entries
        filtered_stock_entries = frappe.qb.from_(se).select(se.name).where(
            se.stock_entry_type.isin(stock_entry_types)
        ).run(pluck="name")
        
        # Apply filter: either not a stock entry, OR is a stock entry that matches our types
        if filtered_stock_entries:
            query = query.where(
                (sle.voucher_type != 'Stock Entry') | 
                ((sle.voucher_type == 'Stock Entry') & (sle.voucher_no.isin(filtered_stock_entries)))
            )
        else:
            # If no stock entries match, exclude all stock entries
