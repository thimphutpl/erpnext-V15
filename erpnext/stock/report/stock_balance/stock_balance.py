...
#         if self.filters.get("summary_based_on") in ["Item Group", "Item Group and Sub Group"]:
#             self.data = self.get_summary_data()
#             self.columns = self.get_summary_columns()
#         else:
#             if not self.columns:
#                 self.columns = self.get_columns()
...
# If your Item Sub Group fieldname is custom_item_sub_group,
...
class StockBalanceReport:
	...
	def get_item_sub_group(self, item_code):
		if not item_code:
			return None

		if item_code not in self.item_sub_group_map:
			self.item_sub_group_map[item_code] = frappe.db.get_value(
				"Item", item_code, ITEM_SUB_GROUP_FIELD
			)

		return self.item_sub_group_map.get(item_code)
	...
	def prepare_opening_data_from_closing_balance(self) -> None:
		...
		for entry in res.data:
			...
			if self.filters.get("item_sub_group"):
				entry_item_sub_group = entry.get("item_sub_group") or self.get_item_sub_group(entry.item_code)
				if entry_item_sub_group != self.filters.get("item_sub_group"):
					continue

				entry["item_sub_group"] = entry_item_sub_group
		...
		item_warehouse_map[group_by_key] = frappe._dict(
			{
				...
				"warehouse": entry.warehouse,
				"item_group": entry.item_group,
				"item_sub_group": entry.get("item_sub_group"),
				"company": entry.company,
				"currency": self.company_currency,
				"stock_uom": entry.stock_uom,
				"item_name": entry.item_name,
	...
	def prepare_stock_ledger_entries(self):
		...
		query = (
			...
			.select(
				...
				sle.has_serial_no,
				item_table.item_group,
				item_table[ITEM_SUB_GROUP_FIELD].as_("item_sub_group"),
				item_table.stock_uom,
				item_table.item_name,
			)
			.where((sle.docstatus < 2) & (sle.is_cancelled == 0))
	...
	def apply_items_filters(self, query, item_table) -> str:
		...
		if item_sub_group := self.filters.get("item_sub_group"):
			query = query.where(item_table[ITEM_SUB_GROUP_FIELD] == item_sub_group)
	...
	def get_summary_data(self):
		summary = {}
		include_sub_group = self.filters.get("summary_based_on") == "Item Group and Sub Group"
		...
		for row in self.data:
			...
			item_group = row.get("item_group") or "Not Set"
			item_sub_group = row.get("item_sub_group") or "Not Set"
			stock_uom = row.get("stock_uom") or ""

			if include_sub_group:
				key = (company, item_group, item_sub_group, stock_uom)
			else:
				key = (company, item_group, stock_uom)

			if key not in summary:
				summary[key] = frappe._dict(
					{
						"company": company,
						"item_group": item_group,
						"item_sub_group": item_sub_group if include_sub_group else "",
						"stock_uom": stock_uom,
		...
		for row in summary.values():
			row["val_rate"] = flt(row.bal_val / row.bal_qty) if row.bal_qty else 0.0

		return list(summary.values())

	def get_summary_columns(self):
		include_sub_group = self.filters.get("summary_based_on") == "Item Group and Sub Group"
		valuation_field_type = self.filters.get("valuation_field_type") or "Currency"
