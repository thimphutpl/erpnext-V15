...
def get_items(filters: StockBalanceFilter) -> List[str]:
    "Get items based on item code, item group or brand."
    if item_code := filters.get("item_code"):
        # Check valuation method for single item selection
        valuation_method = frappe.db.get_value("Item", item_code, "valuation_method")
        if valuation_method == "SPECIFIC":
            return []  # exclude from report
        return [item_code]
        ...
        # Get items first
        items = frappe.get_all("Item", filters=item_filters, pluck="name", order_by=None)

        # EXCLUDE items with valuation_method == "Specific"
        filtered_items = [
            item for item in items
            if frappe.db.get_value("Item", item, "valuation_method") != "SPECIFIC"
        ]
