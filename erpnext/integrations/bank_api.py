...
def intra_payment(from_acc, trans_amount, promo_no, to_acc, unique_transaction_no, remark):
	...
	'Authorization': 'Basic %s' % base64.b64encode(f"{user_id}:{password}".encode("utf-8")).decode("utf-8"),
	'Content-Type': 'application/xml'
	}

	frappe.log_error(title="API Response", message=payload)
	
	response = requests.request("POST", url, headers=headers, data=payload)
	frappe.log_error(title="API Response", message=response.text)
